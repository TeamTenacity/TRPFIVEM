...
local jailPassword = "jail" --change this password to your liking and don't share it with the criminals ;-)
local defaultsecs = 180
local maxsecs = 999999999
		...
		if tablelength(cm) > 2 then
			if cm[2] == jailPassword then
				...
				if GetPlayerName(tPID) ~= nil then
					...
					TriggerClientEvent('chatMessage', -1, 'Police Blotter', { 0, 0, 0 }, GetPlayerName(tPID) ..' jailed for '.. jTmin ..' months. Sentence issued by '..GetPlayerName(source))
				end
			else
				print("Incorrect jailPassword entered by ".. GetPlayerName(source))
			end
