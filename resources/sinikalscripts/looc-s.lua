...
AddEventHandler("chatMessage", function(source, name, message)
    if message:sub(1, 1) == "/" then
        ...
        if cmd == "/o" then
            TriggerClientEvent("sendGlobalMessage2", -1, source, name, tostring(msg))
            CancelEvent()
        end
        ...
        if cmd == "/urpstaffmsg" then
            TriggerClientEvent("sendGlobalMessage3", -1, source, name, tostring(msg))
            CancelEvent()
        end
	--else
