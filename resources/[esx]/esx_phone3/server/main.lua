...
AddEventHandler('esx_phone:startCall', function(number)
  ...
  if foundPlayer ~= false then
    ...
    TriggerClientEvent('esx_phone:endCall', _source, 'This contact is offline')
  end
    ...
    function(result)
      ...
      if foundNumber then
        ...
        if phoneNumber == xPlayer.get('phoneNumber') then
          TriggerClientEvent('esx:showNotification', _source, 'You cant add yourself as a contact!')
        else
          ...
          if hasAlreadyAdded then
            TriggerClientEvent('esx:showNotification', _source, 'This contact is already in your phone!')
          else
            ...
            MySQL.Async.execute(
              ...
              function(rowsChanged)
                ...
                TriggerClientEvent('esx:showNotification', _source, 'Contact added.')
...
AddEventHandler('esx_phone:billCall', function(duration)
  ...
  TriggerClientEvent('esx:showNotification', _source, 'Your call costed you ~r~$' .. amount .. '~s~')
...
AddEventHandler('esx_phone:bankTransfer', function(target, amount)
  ...
  if amount > 0 and amount <= xPlayer.getAccount('bank').money then
    ...
    TriggerClientEvent('esx:showNotification', _source, 'You have sent ~g~$' .. amount .. '~s~ to ' .. targetXPlayer.name)

  else
    TriggerClientEvent('esx:showNotification', _source, 'Amount invalid.')
  end
