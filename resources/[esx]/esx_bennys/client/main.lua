...
AddEventHandler('esx:playerLoaded', function(xPlayer)
  	PlayerData = xPlayer
	ESX.TriggerServerCallback('esx_bennys:getVehiclesPrices', function(vehicles)
		Vehicles = vehicles
	end)
end)
...
RegisterNetEvent('esx_bennys:installMod')
AddEventHandler('esx_bennys:installMod', function()
	--Citizen.Trace('installMod')
	local vehicle = GetVehiclePedIsIn(GetPlayerPed(-1), false)
	myCar = ESX.Game.GetVehicleProperties(vehicle)
	TriggerServerEvent('esx_bennys:refreshOwnedVehicle', myCar)
end)

RegisterNetEvent('esx_bennys:cancelInstallMod')
AddEventHandler('esx_bennys:cancelInstallMod', function()
...
function OpenLSMenu(elems, menuname, menutitle, parent)
	...
	ESX.UI.Menu.Open(
			...
			for k,v in pairs(Config.Menus) do
				if k == data.current.modType or isRimMod then
					if data.current.label == _U('by_default') or string.match(data.current.label, _U('installed')) then
						...
						TriggerEvent('esx_bennys:installMod')
					else
						...
						if isRimMod then
							...
							TriggerServerEvent("esx_bennys:buyMod", price)
		...
		function(data, menu) -- on cancel
			menu.close()
			TriggerEvent('esx_bennys:cancelInstallMod')
				...
				elseif v.modType == 11 or v.modType == 12 or v.modType == 13 or v.modType == 15 or v.modType == 16 then
					...
					for j = 0, modCount, 1 do
						...
						if j == currentMods[k] then
							_label = 'Level ' .. j+1 .. ' - '.. _U('installed') ..''
						else
							price = math.floor(vehiclePrice * v.price[j+1] / 100)
							_label = 'Level ' .. j+1 .. ' - $' .. price .. ' '
						end
						table.insert(elements, {label = _label, modType = k, modNum = j})
