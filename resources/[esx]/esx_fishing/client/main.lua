	...
	if not IsPedInAnyVehicle(GetPed(), false) then
		if not IsPedSwimming(GetPed()) then
			if IsEntityInWater(GetPed()) then
				...
				IsFishing = true
				if ShowChatMSG then ESX.ShowNotification("You have cast your rod, waiting for fish to bite...") end
	...
	while true do Citizen.Wait(1)
		...
		while CFish do
			...
			if TimerAnimation <= 0 then
				...
				if ShowChatMSG then ESX.ShowNotification("The fish got away.") end
			end
