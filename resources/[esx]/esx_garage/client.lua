...
function ListVehiclesMenu()
	...
	ESX.TriggerServerCallback('eden_garage:getVehicles', function(vehicles)
		...
		for _,v in pairs(vehicles) do
    		...
    		labelvehicle = vehicleName..': Impounded'
    		end	
		...
		function(data, menu)
			if(data.current.value.state)then
				...
				TriggerEvent('esx:showNotification', 'Your vehicle has been put in the garage.')
			end
		end,
		function(data, menu)
			menu.close()
...
function reparation(prix,vehicle,vehicleProps)
	...
	local elements = {
		{label = "Return & fix the Vehicle ("..prix.."$)", value = 'yes'},
		{label = "Do not store", value = 'no'},
	}
	ESX.UI.Menu.Open(
		...
		function(data, menu)
			...
			if(data.current.value == 'no') then
				ESX.ShowNotification('Go talk to a ~r~mechanic.~s~')
			end
		...
		ESX.TriggerServerCallback('eden_garage:stockv',function(valid)
			...
			if (valid) then
				...
				if engineHealth < 990 then
			        local fraisRep= math.floor((1000 - engineHealth))			      
...
AddEventHandler('eden_garage:hasEnteredMarker', function(zone)
	...
	if zone == 'spawn' then
		CurrentAction     = 'spawn'
		CurrentActionMsg  = "Press ~INPUT_PICKUP~ to access the garage."
		CurrentActionData = {}
	end

	if zone == 'delete' then
		CurrentAction     = 'delete'
		CurrentActionMsg  = "Press ~INPUT_PICKUP~ to store your vehicle."
		CurrentActionData = {}
	end
