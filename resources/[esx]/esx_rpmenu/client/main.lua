...
function startLoopAnim(lib, anim)
	...
	Citizen.CreateThread(function()
	  ...
	  TaskPlayAnim(GetPlayerPed(-1), lib, anim, 8.0, -8, -1, 49, 0, 0, 0, 0)
...
function checkLoan()
	...
	for i=1, #bills, 1 do
		if bills[i].label == 'Bank Loan' then
			outst = outst + 1
		end
	end

	if outst < 2 then
		TriggerServerEvent('esx_billing:sendBill', serverId, 'society_banker', 'Bank Loan', 5000)
		TriggerServerEvent('esx_rpmenu:getLoan')
		ESX.ShowNotification('Your loan has been ~g~approved!~s~ You can find it in your bank.')
	else
		ESX.ShowNotification('You must pay off all loans before you get a new one! ~y~[F7]')
	end
	end)
end
...
function openRPMenu()
		ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'rpmenu',
		{
			title    = 'Roleplay Options',
			align = 'bottom-right',
			elements = {
				{label = 'Animations', value = 'animate'},
				{label = 'Show Phone Number', value = 'phone'},
				{label = 'Show ID', value = 'id'},

			},
		},
		function(data, menu)
			if data.current.value == 'animate' then
				OpenAnimationsMenu()
			end
			if data.current.value == 'loan' then
				checkLoan()
			end
