					...
					if(IsControlJustPressed(1, 38)) then
						...
						while done == false do
							...
							if(amountToEssence-0.0005 > 0) then
								...
								local essenceToPercent = (essence/0.1073)*65
	...
	while true do
		...
		if(isNearFuelStation and IsPedInAnyVehicle(GetPlayerPed(-1), -1) and not IsPedInAnyHeli(GetPlayerPed(-1)) and not isBlackListedModel() and not isElectricModel() and GetPedVehicleSeat(GetPlayerPed(-1)) == -1) then
			...
			if(menu) then
				...
				local maxEscense = 60-(essence/0.1073)*60
		...
		if(isNearElectricStation() and IsPedInAnyVehicle(GetPlayerPed(-1), -1) and not IsPedInAnyHeli(GetPlayerPed(-1)) and not isBlackListedModel() and isElectricModel() and GetPedVehicleSeat(GetPlayerPed(-1)) == -1) then
			...
			if(menu) then
				...
				local maxEssence = 60-(essence/0.1073)*60
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if(IsPedInAnyVehicle(GetPlayerPed(-1), -1) and GetPedVehicleSeat(GetPlayerPed(-1)) == -1 and not isBlackListedModel()) then
			local kmh = GetEntitySpeed(GetVehiclePedIsIn(GetPlayerPed(-1), false)) * 2.23694
			local vitesse = math.ceil(kmh)

			if(vitesse > 0 and vitesse <2) then
				stade = 0.00000
			elseif(vitesse >= 2 and vitesse <20) then
				stade = 0.00002
	...
	if(IsPedInAnyVehicle(GetPlayerPed(-1), -1) and GetPedVehicleSeat(GetPlayerPed(-1)) == -1 and not isBlackListedModel()) then
		...
		if(hud_form == 1) then
			if(showBar) then
				...
				DrawRect(hud_x, hud_y, 0.0129999999999999, 0.1073,  0, 0 ,0, 70)   
				DrawRect(hud_x, hud_y, 0.0129999999999999, essence, 0, 0, 0, 150)
			end

			if(showText) then
				local percent = (essence/0.1073)*100

				DrawAdvancedText(text_x, text_y, 0.037, -0.0125, 0.50,round(percent,1).."", 255, 255, 255, 255, 0, 1)
			end
		else
			if(showBar) then
				-- DrawRect(hud_x, hud_y, 0.15, 0.0149999999999998, 255, 255, 255, 200)
				DrawRect(hud_x, hud_y, 0.1073, 0.0189999999999999,  0, 0 ,0, 150)
				DrawRect(hud_x, hud_y, essence, 0.0189999999999999, 0, 33, 71, 150)
			end

			if(showText) then
				local percent = (essence/0.1073)*100

				DrawAdvancedText(text_x, text_y, 0.0865, 0.071, 0.35,round(percent,1).."", 255, 255, 255, 255, 0, 1)
			end
		end
	end
end
...
AddEventHandler("essence:hasBuying", function(amount)
	...
	local amountToEssence = (amount/60)*0.1073
...
AddEventHandler("vehicule:sendFuel", function(bool, ess)
	...
	if(bool == 1) then
		...
		essence = (math.random(60,100)/100)*0.1073
...
AddEventHandler("advancedFuel:setEssence", function(percent, plate, model)
	local toEssence = (percent/100)*0.1073
