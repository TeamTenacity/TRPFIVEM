          ...
          if vehFront > 0 and closecar ~= nil and GetPedInVehicleSeat(closecar, -1) ~= GetPlayerPed(-1) then
            ...
            if ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'inventory') then
              ...
              if locked == 1 then
	          	...
	          	ESX.ShowNotification('This vehicle is ~r~locked')
              end
            end
        else
        	ESX.ShowNotification('There are no ~r~vehicles~w~ near you')
          end
      lastOpen = true
...
--[[		for i=1, #weaponList, 1 do
		...
		elseif data.current.value == 'depositw' then
	  		local elem = {}
	  		PlayerData = ESX.GetPlayerData()
--[[			for i=1, #PlayerData.inventory, 1 do
				if PlayerData.inventory[i].count > 0 then
				...
				ESX.UI.Menu.Open(
				  ...
				  function(data4, menu4)
				    ...
				    if quantity > 0 and quantity <= tonumber(data3.current.count) and vehFront > 0 then
				    	local x,y,z = table.unpack(GetEntityCoords(GetPlayerPed(-1),true))
				    	local closecar = GetClosestVehicle(x, y, z, 4.0, 0, 71)
				      TriggerServerEvent('esx_truck_inventory:addInventoryItemW', GetVehicleClass(closecar), GetDisplayNameFromVehicleModel(GetEntityModel(closecar)), GetVehicleNumberPlateText(vehFront), data3.current.value, quantity, data3.current.name)
				    else
			      		ESX.ShowNotification('~r~Quantity invalid.')
				    end
