	...
	while true do Citizen.Wait(1)
		...
		if(IsPedInAnyVehicle(MyPed, false))then
			...
			if HUD.Top then
				...
				if HUD.Plate then
					drawTxt(UI.x + 0.678, 	UI.y + 1.4231, 1.0,1.0,0.55, "~w~" .. PlateVeh, 255, 255, 255, 255)

					drawRct(UI.x + 0.177, 	UI.y + 0.932, 0.054, 0.030, 0,0,0,150)  
				end
				if HUD.ParkIndicator then
					drawRct(UI.x + 0.163, 	UI.y + 0.932, 0.0122, 0.03, 0,0,0,150)

					if VehStopped then
						drawTxt(UI.x + 0.6646, UI.y + 1.422, 1.0,1.0,0.6, "~r~P", 255, 255, 255, 200)
					else
						drawTxt(UI.x + 0.6646, UI.y + 1.422, 1.0,1.0,0.6, "P", 255, 255, 255, 150)
					end
				end
			end
			if HUD.SpeedIndicator then
				drawRct(UI.x + 0.2331, 	UI.y + 0.932, 0.0373,0.03,0,0,0,150) -- Speed panel
				if HUD.Speed == 'kmh' then
					drawTxt(UI.x + 0.40, 	UI.y + 1.42, 1.0,1.0,0.64 , "~w~" .. math.ceil(Speed), 255, 255, 255, 255)
					drawTxt(UI.x + 0.6333, 	UI.y + 1.432, 1.0,1.0,0.4, "~w~ km/h", 255, 255, 255, 255)
				elseif HUD.Speed == 'mph' then
					drawTxt(UI.x + 0.734, 	UI.y + 1.421, 1.0,1.0,0.64 , "~w~" .. math.ceil(Speed), 255, 255, 255, 255)
					drawTxt(UI.x + 0.750, 	UI.y + 1.432, 1.0,1.0,0.4, "~w~ mph", 255, 255, 255, 255)
				else
